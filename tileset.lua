...
function Tileset:new(image, tileLayer, firstGid)
    local tileset = {
        tiles = {},
        spritebatch = love.graphics.newSpriteBatch(image, 6000)
    }
    ...
    for i = 1, #tileData, 1 do
        ...
        if tile ~= 0 then
            ...
            local qcoords = getTileCoords(tile-firstGid+1, image)
    ...
    self.__index = self
    return setmetatable(tileset, self)
end
